...
function RemplirTabRestoChatName(map1resto, map2resto) {
  map1resto.forEach(element => {
    console.log(element);
  });

  map2resto.forEach(element => {
    console.log(element);
  });
...
async function Main() {
  let promise = new Promise((resolve, reject) => {
    getArrayFromRelais(loadRelais, resolve);
  });
  let re = await promise;
  //console.log(relaisArray.length);

  let promise2 = new Promise((resolve, reject) => {
    getArrayFromMichelin(loadMichelin, resolve);
  });
  let re2 = await promise2;

  let promise3 = new Promise((resolve, reject) => {
    getRestaurants(relaisArray, loadRestRelais, resolve);
  });
